...
def two_way_compare(reports, diff_csv):
  ...
  for keys in report_a.gen_keys_list():
    ...
    result_a = report_a.get_test_status(module_name, abi, class_name, test_name)
    tests[test_name] = [result_a, parse_cts_report.NO_DATA]

  for keys in report_b.gen_keys_list():
    module_name, abi, class_name, test_name = keys
    ...
    result_b = report_b.get_test_status(module_name, abi, class_name, test_name)

    if test_name not in tests:
      tests[test_name] = [parse_cts_report.NO_DATA, result_b]
    else:
      tests[test_name][1] = result_b
