...
class Soong:
    ...
    def configure(self, additional_config: Dict[str, Any]) -> None:
        ...
        Note that, while Soong does have defaults, the defaults are not used if custom
        configuration is required. As such, calling this function even with an empty
        argument may cause Soong to run with different defaults than if the function
        were not called, as we must populate the configuration with our own "defaults",
        that may not be complete or up-to-date with Soong's.

        The out directory will be created if it does not already exist. Existing
        contents will not be removed, but soong.variables will be overwritten.

        Args:
            additional_config: A JSON-compatible dict that will be merged into the
        ...
        all_codenames = self.get_make_var("PLATFORM_VERSION_ALL_CODENAMES").split(",")
