...
impl<CollectionType: NameAndVersionMap> VersionMatch<CollectionType> {
    ...
    pub fn pairs<'a>(&'a self) -> impl Iterator<Item = VersionPair<'a, CollectionType::Value>> {
        self.source
            .map_field()
            .iter()
            .map(|(nv, source)| VersionPair { source, dest: self.get_compatibile_item(nv) })
    }
...
mod tests {
    ...
    fn test_version_map() -> Result<()> {
        ...
        assert_eq!(version_match.get_compatibile_item(&equal), Some(&"equal dest".to_string()));
        assert_eq!(
            version_match.get_compatibile_item_mut(&equal),
        ...
        assert_eq!(
            version_match.get_compatibile_item(&compatible_old),
            Some(&"compatible dest".to_string())
        );
        assert_eq!(
            version_match.get_compatibile_item_mut(&compatible_old),
        ...
        assert!(version_match.get_compatibile_item(&incompatible_old).is_none());
        assert!(version_match.get_compatibile_item_mut(&incompatible_old).is_none());
        ...
        assert!(version_match.get_compatibile_item(&downgrade_old).is_none());
        assert!(version_match.get_compatibile_item_mut(&downgrade_old).is_none());
        ...
        assert!(version_match.get_compatibile_item(&missing).is_none());
        assert!(version_match.get_compatibile_item_mut(&missing).is_none());
        ...
        assert!(version_match.get_compatibile_item(&superfluous).is_none());
        assert!(version_match.get_compatibile_item_mut(&superfluous).is_none());
