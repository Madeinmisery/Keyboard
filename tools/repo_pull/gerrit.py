...
def _query_change_lists(url_opener, gerrit, query_string, start, count):
  """Query change lists from start to count."""
  data = [
      ('q', query_string),
      ('o', 'CURRENT_REVISION'),
      ('o', 'CURRENT_COMMIT'),
...
def query_change_lists(url_opener, gerrit, query_string, start, count):
  """Query change lists from start to count, handle _more_changes from server."""
  changes = []
...
def main():
    ...
    if args.verbose:
        ...
        txt = '{i:<8} {number:<16} {status:<20} {change_id:<60} {project}'
