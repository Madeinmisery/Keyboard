...
def _query_change_lists(url_opener, gerrit, query_string, start, count):
    ...
    returned for the _more_changes attribute to determine if more changes
    are available and perform additional queries adjusting the start index.
...
def query_change_lists(url_opener, gerrit, query_string, start, count):
    ...
    This function queries the Gerrit server based on the input parameters for
    a list of changes.  This function handles querying the server multiple
    times if necessary and combining the results that are returned to the
    caller.
    ...
    while len(changes) < count:
        ...
        # The last change object contains a _more_changes attribute if
        # the number of changes exceeds the query parameter or the
        # internal server limit.   Stop iteration if `_more_changes`
