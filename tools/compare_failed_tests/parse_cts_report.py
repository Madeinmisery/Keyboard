...
def write_to_csv(test_result, result_csvfile, summary_csvfile):
  ...
  for module_name, abis in modules.items():
    ...
    for abi_name, testcases in abis.items():
      ...
      summary = [value for value in module_result_summary.counter.values()]
...
def main():
  ...
  with (
      open(os.path.join(output_dir, 'result.csv'), 'w') as result_csvfile,
      open(os.path.join(output_dir, 'summary.csv'), 'w') as summary_csvfile,
  ):
    write_to_csv(test_result, result_csvfile, summary_csvfile)

  parsed_info = os.path.join(output_dir, 'info.json')
  parsed_result = os.path.join(output_dir, 'result.csv')
  parsed_summary = os.path.join(output_dir, 'summary.csv')

  for f in [parsed_info, parsed_result, parsed_summary]:
