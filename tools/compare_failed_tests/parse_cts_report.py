...
turns them into a set of information files.
"""
...
class CtsReport:
  ...
  def set_test_status(self, module_name, abi,
    ...
    if not previous:
      ...
      if (CtsReport.STATUS_ORDER.index(test_status)
  ...
  def output_files(self, output_dir):
    ...
    sub_dir_name = os.path.join(output_dir, self.info['build_model'])

    if os.path.exists(sub_dir_name):
      raise FileExistsError(f'Output directory {sub_dir_name} already exists.')

    os.mkdir(sub_dir_name)

    parsed_info_path = os.path.join(sub_dir_name, 'info.json')
    parsed_result_path = os.path.join(sub_dir_name, 'result.csv')
    parsed_summary_path = os.path.join(sub_dir_name, 'summary.csv')
  ...
  def write_to_csv(self, result_csvfile, summary_csvfile):
    ...
    for module_name, abis in modules.items():
      for abi_name, test_classes in abis.items():
        module_summary = self.module_summaries[module_name][abi_name]
...
def main():
  ...
  report = parse_report_file(report_file, output_dir)
