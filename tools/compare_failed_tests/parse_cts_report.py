...
def extract_xml_from_zip(zip_file_path, dir_path):
  ...
  subfolder = pathlib.Path(zip_file_path).stem
  xml_path = os.path.join(subfolder, 'test_result.xml')
  with zipfile.ZipFile(zip_file_path) as myzip:
    myzip.extract(xml_path, path=dir_path)
  shutil.copyfile(os.path.join(dir_path, xml_path),
                  os.path.join(dir_path, 'test_result.xml'))
  shutil.rmtree(os.path.join(dir_path, subfolder))
...
def main():
  ...
  file_extension = os.path.splitext(args.report_files[0])[1]

  xml_path = ''
  if file_extension == '.zip':
    extract_xml_from_zip(args.report_files[0], dir_path)
  else:
    shutil.copyfile(args.report_files[0],
                    os.path.join(dir_path, 'test_result.xml'))
