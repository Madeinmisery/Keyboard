...
def get_result(test_result, module_name, testcase_name, test_name):
  ...
  return ([y for x, y in tests[test_name].items()],
          ', '.join([x + ': ' + y for x, y in tests[test_name].items()]))
...
def write_to_csv(test_result, result_csvfile, summary_csvfile):
  """Given a result dict, list all test results and write to the csv files.
  ...
  summary_writer.writerow(['module', 'pass', 'fail', 'IGNORED',
                           'ASSUMPTION_FAILURE', 'TEST_ERROR', 'TEST_STATUS_UNSPECIFIED'])
  ...
  for module_name, testcases in modules.items():
    ...
    for testcase_name, tests in testcases.items():
      ...
      for test_name, result in tests.items():
        result_list, result_str = get_result(test_result, module_name, testcase_name, test_name)

        testcase_sub_summary += '    ' + test_name + ': ' + result_str + '\n'
        result_writer.writerow([module_name, testcase_name, test_name, result_str])
    ...
    summary_writer.writerow([module_name] + [value for value in module_result_summary.counter.values()])
