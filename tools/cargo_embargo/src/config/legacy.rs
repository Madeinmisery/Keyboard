...
impl Config {
    ...
    const FIELD_NAMES: [&str; 4] = ["add_toplevel_block", "dependencies", "patch", "run"];
    ...
    pub fn from_json_str(json_str: &str) -> Result<Self> {
        ...
        for variant in &mut variants {
            ...
            for (key, value) in &config {
                if !Self::FIELD_NAMES.contains(&key.as_str()) && !variant.contains_key(key) {
...
impl VariantConfig {
    ...
    pub fn to_embargo(&self, package_name: &str, run_cargo: bool) -> Result<super::VariantConfig> {
        ...
        if let Some(suffix) = &self.suffix {
            let module_name = package_to_library_name(package_name);
            let with_suffix = format!("{}{}", module_name, suffix);
