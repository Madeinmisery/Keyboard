...
/// Header lines to prepend out generated build files
pub static BUILD_FILE_HEADER: [&str; 2] = [
...
fn write_build_files(
    ...
    if !bp_contents.is_empty() {
        ...
        let contents = BUILD_FILE_HEADER
            ...
            + "\n"
            + &bp_contents;
        write_format_android_bp(&output_path, &contents, package_cfg.patch.as_deref())?;
    }
    if !mk_contents.is_empty() {
        ...
        let contents = BUILD_FILE_HEADER
            ...
            .join("\n")
            + "\n"
            + "# Content before the line starting with LOCAL_DIR is preserved.\n"
            + "\n"
            + read_license_header(&output_path)?.trim()
            + "\n"
            + &mk_contents;
