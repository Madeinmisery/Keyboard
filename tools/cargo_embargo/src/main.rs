...
fn run_embargo(args: &Args, config_filename: &Path) -> Result<()> {
    ...
    write_all(&cfg, crates, &package_out_files)
}
...
fn write_build_files(
    ...
    if !mk_contents.is_empty() {
        ...
        if let Some(patch) = package_cfg.rulesmk_patch.as_deref() {
            patch_rules_file(&output_path, patch)?;
        }
    }
...
fn generate_rules_mk(
    ...
    for c in crates {
        ...
        return crate_to_rulesmk(c, cfg, package_cfg, &extra_srcs).with_context(|| {
            format!(
    ...
    Err(anyhow!("No valid crate found for package {package_name}"))
}

/// Apply patch from `patch_path` to file `output_path`.
///
/// Warns but still returns ok if the patch did not cleanly apply,
fn patch_rules_file(output_path: &Path, patch_path: &Path) -> Result<()> {
...
fn write_format_android_bp(
    ...
    if let Some(patch_path) = patch_path {
        patch_rules_file(bp_path, patch_path)?;
