...
def start_gdbserver(device, gdbserver_local_path, gdbserver_remote_path,
    ...
    if gdbserver_local_path is not None:
        ...
        # If the user here is potentially on Windows or macOS, adb may not be
        # able to inspect execute permissions. Since we don't know where the
        # users are, chmod gdbserver_remote_path on device regardless.
