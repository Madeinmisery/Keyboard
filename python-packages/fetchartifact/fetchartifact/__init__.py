...
def _make_query_url(
    ...
    """Constructs the URL used for querying the download URL.

    The download URL itself is not stable, but there's an API that will return the
    download URL. This function constructs the URL of that API.
    ...
    Returns:
        URL of the download location API for the given artifact.
    """
    return (
...
async def _get_download_url(
    ...
    query_url = _make_query_url(target, build_id, artifact_name, query_url_base)
    _logger().debug("Fetching download URL from %s", query_url)
    async with session.get(query_url) as response:
        response.raise_for_status()
        return cast(str, (await response.json())["signedUrl"])
...
async def fetch_artifact(
    ...
    download_url = await _get_download_url(
        target, build_id, artifact_name, session, query_url_base
    )
