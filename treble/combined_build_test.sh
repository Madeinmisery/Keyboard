...
usage() {
    echo "usage: ${0} -t TARGET -v VARIANT [-d DIST_OUT] [-a ALTER_TARGET] [-c] [-o RESULTS] [-r] [GOALS ...]"
    ...
    echo "  -o RESULTS     : Write build time results to RESULT file"
    echo "  -r             : Dryrun to see the commands without actually building the targets"
}

while getopts ha:cd:o:rt:v: opt; do
    case "${opt}" in
        h)
            usage
            ;;
...
write_output() {
    if [[ -z "${result_out}" || ! -z "${dry_run}" ]]; then
    echo "Output: $1"
    else
