...
class GetRefDumpDirStem:
    def __init__(self, ref_dump_dir, chosen_vndk_version, chosen_platform_version, binder_bitness):
    ...
    def __call__(self, category, arch):
        version_stem = self.chosen_vndk_version if category == 'vndk' else self.chosen_platform_version
...
def remove_references_for_all_arches(get_ref_dump_dir_stem, targets, libs):
    for target in targets:
        for category in ('ndk', 'platform', 'vndk'):
            dir_to_remove = get_ref_dump_dir_stem(category, target.get_arch_str())
            if libs:
                for lib in libs:
...
def find_and_copy_lib_lsdumps(get_ref_dump_dir_stem, target, libs, lsdump_paths, compress):
...
def create_source_abi_reference_dumps(args, get_ref_dump_dir_stem, lsdump_paths, targets):
    num_libs_copied = 0
    for target in targets:
...
def create_source_abi_reference_dumps_for_all_products(args):
    ...
    for product in args.products:
        ...
        get_ref_dump_dir_stem = GetRefDumpDirStem(args.ref_dump_dir, chosen_vndk_version, chosen_platform_version, binder_bitness)
        ...
        remove_references_for_all_arches(get_ref_dump_dir_stem, targets, args.libs)
        ...
        num_processed += create_source_abi_reference_dumps(
            args, get_ref_dump_dir_stem, lsdump_paths,
            targets)
