...
class GetVersionedRefDumpDirStem:
    def __init__(self, chosen_vendor_version, chosen_platform_version,
                 binder_bitness):
        self.chosen_vendor_version = chosen_vendor_version
    ...
    def __call__(self, subdir, arch_str):
        ...
        version_stem = (self.chosen_vendor_version
                        if subdir == 'vndk'
...
def create_source_abi_reference_dumps_for_all_products(args):
    ...
    for product in args.products:
        ...
        # In U version, RELEASE_BOARD_API_LEVEL is undefined;
        # PLATFORM_VNDK_VERSION is 34 or UpsideDownCake.
        # In later versions, RELEASE_BOARD_API_LEVEL is in YYYYMM format.
        chosen_vendor_version = (release_board_api_level or
        ...
        if args.ref_dump_dir:
            ...
            if not chosen_vendor_version:
                raise RuntimeError('Neither PLATFORM_VNDK_VERSION nor '
                                   'RELEASE_BOARD_API_LEVEL is defined.')
            get_ref_dump_dir_stem = GetVersionedRefDumpDirStem(
                chosen_vendor_version, chosen_platform_version, binder_bitness)
