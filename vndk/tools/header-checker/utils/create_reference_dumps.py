...
def get_ref_dump_dir_stem(ref_dump_dir, category, chosen_vndk_version, platform_version_codename,
                          binder_bitness, arch):
    if category != 'vndk' and 'REL':
        return os.path.join(ref_dump_dir, category, 'current',
                            binder_bitness, arch)
    else:
...
def remove_references_for_all_arches(ref_dump_dir, chosen_vndk_version, platform_version_codename,
                                     binder_bitness, targets, libs):
    for target in targets:
        for category in ('ndk', 'platform', 'vndk'):
            dir_to_remove = get_ref_dump_dir_stem(
                ref_dump_dir, category, chosen_vndk_version, platform_version_codename, binder_bitness,
                target.get_arch_str())
            if libs:
                for lib in libs:
...
def find_and_copy_lib_lsdumps(ref_dump_dir, chosen_vndk_version, platform_version_codename,
    ...
    for tag, path in arch_lsdump_paths:
        ref_dump_dir_stem = get_ref_dump_dir_stem(
            ref_dump_dir, tag_to_dir_name(tag), chosen_vndk_version, platform_version_codename,
...
def create_source_abi_reference_dumps(args, chosen_vndk_version, platform_version_codename,
    ...
    for target in targets:
        ...
        num_libs_copied += find_and_copy_lib_lsdumps(
            args.ref_dump_dir, chosen_vndk_version, platform_version_codename, binder_bitness, target,
...
def create_source_abi_reference_dumps_for_all_products(args):
    ...
    for product in args.products:
        ...
        remove_references_for_all_arches(
            args.ref_dump_dir, chosen_vndk_version, platform_version_codename, binder_bitness, targets,
            args.libs)
        ...
        num_processed += create_source_abi_reference_dumps(
            args, chosen_vndk_version, platform_version_codename, binder_bitness, lsdump_paths, targets)
