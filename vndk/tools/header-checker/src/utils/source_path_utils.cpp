...
static const std::vector<std::string> header_extensions {".h", ".hh", ".hpp",
  ".hxx", ".h++", ".inl", ".inc", ".ipp", ".h.generic"};
...
static bool ShouldSkipFile(llvm::StringRef &file_name) {
  ...
  const std::string ext = file_name.rsplit('.').second;
  return std::find_if(header_extensions.begin(), header_extensions.end(),
    [ext](std::string e){return ext.endswith(e);})
    == array.end();
}
