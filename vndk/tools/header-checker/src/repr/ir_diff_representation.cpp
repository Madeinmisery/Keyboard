...
bool VTableLayoutDiffIR::IsExtended() const {
  ...
  while (old_it != old_end) {
    ...
    if (new_it == new_end || old_it == new_end) {
      return false;
    }
    ...
    new_it++;
    // Compare RTTI.
    if (old_it == old_end || old_it->GetKind() != VTableComponentIR::RTTI ||
        new_it == new_end || new_it->GetKind() != VTableComponentIR::RTTI ||
  ...
  return old_it == old_end ? is_extended : false;
}
