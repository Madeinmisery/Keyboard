...
struct Qualifiers {
  ...
  bool IsMoreQualifiedThan(const Qualifiers &other) const {
    return ((is_const && !other.is_const) ||
            (is_restricted && !other.is_restricted) ||
            (is_volatile && !other.is_volatile));
  }
...
// casted. The rules are described on
// https://en.cppreference.com/w/cpp/language/implicit_conversion#Qualification_conversions
// Additionally, __restrict__ follows the same rule as const and volatile.
static bool ResolveImplicitlyConvertibleQualifiedReferences(
  ...
  while (true) {
    ...
    if (is_first_level) {
      ...
      if (old_qual.IsMoreQualifiedThan(new_qual)) {
        return false;
      }
      if (!is_const_since_second_level &&
          new_qual.IsMoreQualifiedThan(old_qual)) {
        return false;
      }
