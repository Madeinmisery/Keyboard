...
//   The old field has unique offset in old_fields.
//   The new field has unique offset in new_fields.
//   The two fields have equal attributes except the name.
void AbiDiffHelper::FilterOutRenamedRecordFields(
  ...
  auto is_offset_less = [](const RecordFieldIR *first,
                           const RecordFieldIR *second) {
    return first->GetOffset() < second->GetOffset();
  };
  std::sort(old_fields.begin(), old_end, is_offset_less);
  std::sort(new_fields.begin(), new_end, is_offset_less);
  ...
  while (old_it != old_end && new_it != new_end) {
    auto next_old_it = old_it;
    while (next_old_it != old_end &&
           (*old_it)->GetOffset() == (*next_old_it)->GetOffset()) {
      next_old_it++;
    }
    if ((*old_it)->GetOffset() < (*new_it)->GetOffset() ||
        next_old_it - old_it > 1) {
      out_old_fields.insert(out_old_fields.end(), old_it, next_old_it);
      old_it = next_old_it;
      continue;
    }

    auto next_new_it = new_it;
    while (next_new_it != new_end &&
           (*new_it)->GetOffset() == (*next_new_it)->GetOffset()) {
      next_new_it++;
    }
    if ((*old_it)->GetOffset() > (*new_it)->GetOffset() ||
        next_new_it - new_it > 1) {
...
AbiDiffHelper::CompareRecordFields(
    ...
    std::deque<std::string> *type_queue,
    DiffMessageIR::DiffKind diff_kind) {
  GenericFieldDiffInfo<RecordFieldIR, RecordFieldDiffIR> result;
  ...
  result.removed_fields_ =
      utils::FindRemovedElements(old_fields_map, new_fields_map);
  result.added_fields_ =
      utils::FindRemovedElements(new_fields_map, old_fields_map);
  FilterOutRenamedRecordFields(type_queue, diff_kind, result.removed_fields_,
                               result.added_fields_);
  ...
  for (auto &&common_fields : cf) {
    ...
    if (diffed_field_ptr.second != nullptr) {
      result.diffed_fields_.emplace_back(
          std::move(*(diffed_field_ptr.second.release())));
    }
  }
  // Determine DiffStatus.
  DiffStatus &diff_status = result.diff_status_;
  diff_status = DiffStatus::kNoDiff;
  if (result.diffed_fields_.size() != 0 || result.removed_fields_.size() != 0) {
    diff_status.CombineWith(DiffStatus::kDirectDiff);
  }
  if (result.added_fields_.size() != 0) {
