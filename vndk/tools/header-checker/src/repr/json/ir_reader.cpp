...
void JsonIRReader::ReadTypeInfo(const JsonObjectRef &type_decl,
  ...
  std::string self_type = type_decl.GetString("self_type");
  type_ir->SetSelfType(self_type.size() > 0 ? self_type
                                            : type_ir->GetLinkerSetKey());
  // ReferencesOtherType
  std::string referenced_type = type_decl.GetString("referenced_type");
  type_ir->SetReferencedType(
      referenced_type.size() > 0 ? referenced_type : type_ir->GetSelfType());
}
...
void JsonIRReader::ReadGlobalVariables(const JsonObjectRef &tu) {
  for (auto &&global_variable : tu.GetObjects("global_vars")) {
    ...
    // ReferencesOtherType
    std::string referenced_type = global_variable.GetString("referenced_type");
    global_variable_ir.SetReferencedType(
        referenced_type.size() > 0 ? referenced_type
                                   : global_variable_ir.GetLinkerSetKey());
