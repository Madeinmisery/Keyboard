...
static llvm::cl::opt<bool> current(
    "current",
    llvm::cl::desc("Diff with current version dump"),
    llvm::cl::init(false), llvm::cl::Optional,
    llvm::cl::cat(header_checker_category));

static llvm::cl::opt<std::string> sdk_version(
    "sdk-version",
    llvm::cl::desc("Referenced dump sdk version"),
...
static void ReadConfigFile(const std::string &config_file_path) {
  ...
  if (cfg.HasSection("global", "")) {
    UpdateFlags(cfg.GetSection("global", ""));
  }
  if (current && cfg.HasSection(lib_name, "current")) {
    UpdateFlags(cfg.GetSection(lib_name, "current"));
  } else if (cfg.HasSection(lib_name, sdk_version)){
    UpdateFlags(cfg.GetSection(lib_name, sdk_version));
  }
}
