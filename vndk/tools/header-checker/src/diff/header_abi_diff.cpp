...
static llvm::cl::list<std::string> ignored_linker_set_keys(
    "ignored-linker-set-key",
    llvm::cl::desc("Ignore specific symbol in the comparison."),
...
static void UpdateFlags(const ConfigSection &section) {
  for (auto &&i : section.GetIgnoredLinkerSetKeys()) {
    ignored_linker_set_keys_set.insert(i);
  }
  for (auto &&p : section) {
    auto &&key = p.first;
...
int main(int argc, const char **argv) {
  ...
  for (auto &&i : ignored_linker_set_keys) {
    ignored_linker_set_keys_set.insert(i);
  }
