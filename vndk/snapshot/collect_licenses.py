...
class LicenseCollector(object):
    ...
    Initialize the the LicenseCollector with a vndk snapshot directory.
    ...
    def read_and_check_licenses(self, file_path, license_keywords):
        ...
        for (root, _, files) in os.walk(path):
            if len(files) > 0:
                for f in files:
                    ...
                    self.read_and_check_licenses(file_path, LICENSE_KEYWORDS)
                    if self.read_and_check_licenses(file_path, RESTRICTED_LICENSE_KEYWORDS):
