...
assert "trusty/host/common/scripts" in os.path.dirname(__file__)
TRUSTY_TOP = os.path.abspath(
...
class Crate(object):
    ...
    def __init__(self, runner, outf_name):
        ...
        self.externs = []  # follows --extern
        self.core_externs = []  # first part of self.externs elements
        ...
        self.dependencies = []  # list of crate dependencies
        self.feature_dependencies = {}  # maps features to optional dependencies
    ...
    def dump_trusty_module(self):
        ...
        if "test" in self.crate_types:
            ...
            return
        raise NotImplementedError("Multiple crate types not supported")
    ...
    def dump_srcs_list(self):
        ...
        # TODO(perl): is there a need to support more than one output file?
        if srcdeps := [
    ...
    def dump_one_trusty_module(self, crate_type):
        ...
        if crate_type == "test":  # TODO: support test crates
            print(f"### WARNING: ignoring test crate: {self.crate_name}")
            return
        self.dump_core_properties()
        if not self.defaults:
            self.dump_edition_flags_libs()

        # TODO(perl): improve and clean up dependency handling
        library_deps = []
        for dependency in self.dependencies:
            ...
            abspath = os.path.normpath(os.path.join(TRUSTY_TOP, path))
            ...
            if os.path.isdir(abspath):
                ...
                if feats := [
                    ...
                    if name in self.feature_dependencies[f]
                ]:
                    print(
                        f"ERROR: missing dependency {name} needed by features "
                        + ", ".join(feats)
                    )
                    return
            else:
                print(f"ERROR: missing non-optional dependency: {path}")
                return
        if library_deps:
        ...
        if crate_type in LIBRARY_CRATE_TYPES:
            ...
            raise NotImplementedError("Unsupported crate type: " + crate_type)
    ...
    def dump_edition_flags_libs(self):
        ...
        # TODO(perl): do we need to do something with self.externs?
        if [
            lib
            for lib in self.static_libs
            if not lib in self.runner.args.lib_blocklist
        ]:
            raise NotImplementedError("Crate with static libs")
    ...
    def dump_core_properties(self):
        ...
        # Trusty's module system only supports rlib so we make lib crates rlibs
...
class Runner(object):
    ...
    def setup_cargo_path(self):
        ...
        if self.args.cargo_bin:
            ...
            return
        # TODO(perl): try getting cargo from $RUST_BINDIR set in envsetup.sh
        # We have only tested this on Linux.
        if platform.system() != "Linux":
            sys.exit(
                "ERROR: this script has only been tested on Linux with cargo."
            )
        # Assuming that this script is in trusty/host/common/scripts.
        my_dir = os.path.dirname(os.path.abspath(__file__))
        linux_dir = os.path.join(
            my_dir, "..", "..", "..", "..", "prebuilts", "rust", "linux-x86"
        )
