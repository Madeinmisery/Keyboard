  ...
  def parse(self, line_num, line):
    ...
    while i < len(args):
      ...
      if arg == '--crate-name':
        ...
        cargo_metadata = subprocess.run("cargo metadata --no-deps", cwd=os.path.abspath(self.cargo_dir), shell=True, capture_output=True)
        if cargo_metadata.returncode:
            self.errors += "ERROR: unable to get cargo metadata for package version; return code " + cargo_metadata.returncode + "\n"
        else:
