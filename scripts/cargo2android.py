...
class Crate(object):
  ...
  def __init__(self, runner, outf_name):
    ...
    self.feature_suffix = ''
    if "feature_builds" in self.runner.args:
      self.feature_suffix = self.runner.args.feature_builds[self.runner.feature_build_num]['feature-suffix']
  ...
  def decide_one_module_type(self, crate_type):
    ...
    if crate_type == 'bin':  # rust_binary[_host]
      ...
      self.stem = altered_stem(self.crate_name) + self.feature_suffix
      self.module_name = altered_name(self.stem)
      ...
      self.stem = 'lib' + self.crate_name + self.feature_suffix
      self.module_name = altered_name(self.stem)
...
class Runner(object):
  ...
  def run_cargo(self):
    ...
    if self.skip_cargo:
      return self
    num_builds = len(self.args.feature_builds) if 'feature_builds' in self.args else 1
    for feature_build_num in range(num_builds):
      cargo_toml = './Cargo.toml'
      cargo_out = './cargo.out'
      if feature_build_num > 0:
        cargo_out = f'./cargo_{feature_build_num}.out'
      ...
      for c in self.cargo:
        features = ''
        if c != 'clean':
          if 'feature_builds' in self.args:
            self.args.features = self.args.feature_builds[feature_build_num]['features']
  ...
  def gen_bp(self):
    ...
    if self.dry_run:
      ...
      for feature_build_num in range(num_builds):
        ...
        if feature_build_num > 0:
          cargo_out_path = f'./cargo_{feature_build_num}.out'
        self.feature_build_num = feature_build_num
