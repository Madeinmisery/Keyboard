  ...
  def ProcessLine(self, line):
    ...
    if trace_line_dict is not None:
      ...
      if area == "<unknown>" or area == "[heap]" or area == "[stack]":
        ...
        # If we know the build_id, do a brute-force search of the symbols directory.
        if not os.path.isfile(symbol.SYMBOLS_DIR + lib) and build_id:
          lib = self.GetLibraryByBuildId(symbol.SYMBOLS_DIR, os.path.basename(lib), build_id) or lib
