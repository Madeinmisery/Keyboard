...
if __name__ == '__main__':
  ...
  for lib in os.sys.argv[1:]:
    stdout = subprocess.run(["objdump", "-t", lib], stdout=subprocess.PIPE, check=True).stdout
    addrs = re.findall("(?<=^)[0-9a-f]{8,}", stdout.decode("utf8"), re.MULTILINE)
    syms = CallLlvmSymbolizerForSet(lib, set(addrs))
    with open("old.txt", "wt") as o, open("new.txt", "wt") as n:
      for addr, name in syms.items():
        old = name[0][0]
        new = FormatSymbolWithoutParameters(old)
        if old != new:
          o.write(addr + "\n")
          o.write(old + "\n")
          n.write(addr + "\n")
