...
pub fn parse_cargo_out(cargo_out_path: &str, cargo_metadata_path: &str) -> Result<Vec<Crate>> {
    ...
    for rustc in cargo_out.rustc_invocations.iter() {
        ...
        //
        // Canonicalize to get rid of symlinks (e.g. in crosvm there are some symlinks from
        // crosvm's own 3p directory to the android 3p directories. We don't want to modify those).
        let package_dir = c.package_dir.canonicalize().unwrap_or_else(|_| c.package_dir.clone());
        if !package_dir.starts_with(std::env::current_dir().unwrap()) {
            continue;
        }
        crates.push(c);
    }
